   ...
   private static final String[] DEFAULT_SCHEMES = {"http", "https", "ftp"}; // Must be lower-case
    ...
    public UrlValidator(String[] schemes, RegexValidator authorityValidator, long options) {
        ...
        if (isOn(ALLOW_ALL_SCHEMES)) {
            ...
            for(int i=0; i < schemes.length; i++) {
                allowedSchemes.add(schemes[i].toUpperCase(Locale.ENGLISH));//This is the error
                //Needs to be allowedSchemes.add(schemes[i].toLowerCase(Locale.ENGLISH));
    ...
    public boolean isValid(String value) {
        ...
        if ("http".equals(scheme)) {//error // Special case - file: allows an empty authority
            //Needs to be if("file".equals(scheme))
            if (authority != null) {
